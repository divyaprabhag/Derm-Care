...
const Navbar = ({ onSearch, cartItems, onCartClick }) => {
  ...
  const { email, role, logout } = useAuth(); // ✅ Use context
    ...
    <nav className="bg-gray-900 p-6">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex space-x-6 items-center">
          ...
          {email && (
            <button onClick={handleLogout} className="text-gray-400 hover:text-pink-400">
              Logout
            </button>
          )}

          {role === "admin" && (
            <NavLink to="/admin" className="text-yellow-400 hover:text-yellow-500 font-semibold">
              Admin Panel
            </NavLink>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          />
          <svg className="w-6 h-6 text-gray-400 hover:text-pink-400 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24">
