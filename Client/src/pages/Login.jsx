...
//     if (!name || !email || !phone) {
//       setErrorMessage("Please fill in name, email, phone.");
//       setIsSubmitted(false);
//       return;
//     }

//     try {
//       // Legacy save (keeps existing behavior)
//       await axios.post("http://localhost:5000/api/login", { name, email, phone });

//       // If user provided a password, try secure auth -> /api/auth/login
//       if (password) {
//         try {
...
//           return;
//         } catch (err) {
//           setErrorMessage(err.response?.data?.message || 'Secure login failed. Check credentials.');
//           setIsSubmitted(false);
//           return;
//         }
//       }

//       // No password: keep legacy behaviour, ask user to signup for secure access
//       setIsSubmitted(true);
//       setErrorMessage('Saved legacy login. To access protected parts, please signup with a password.');
...
//           <input type="email" placeholder="Email" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
//           <input type="tel" placeholder="Phone" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
//           <input type="password" placeholder="Password (optional — enter for secure login)" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
//           <button type="submit" className="w-full bg-white text-[linear-gradient(to_right,#414345,#232526)] p-3 rounded-lg hover:bg-pink-500 transition"> Submit </button>
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // legacy save — harmless
      await axios.post("http://localhost:5000/api/login", { name, email, phone });

      // If password provided, do secure login
      if (password) {
        const res = await axios.post("http://localhost:5000/api/auth/login", { email, password });
        const { token, user } = res.data;
        login(token, user);
      ...
      // If no password, keep legacy behavior
      setIsSubmitted(true);
      setErrorMessage('Saved legacy login. To access protected parts, please signup/login with a password.');
      setTimeout(() => setIsSubmitted(false), 3000);
    } catch (error) {
      setErrorMessage(error?.response?.data?.message || "Failed to submit. Try again.");
      console.error(error);
    }
  };
    ...
    <div className="min-h-screen bg-white flex justify-center items-center pb-16">
      <div className="bg-[linear-gradient(to_right,#414345,#232526)] p-10 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <input type="email" placeholder="Email" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
          <input type="tel" placeholder="Phone" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
          <input type="password" placeholder="Password (enter for secure login)" className="w-full p-3 border-2 border-slate-500 rounded-lg focus:outline-none focus:border-black" />
          <button type="submit" className="w-full bg-white text-[linear-gradient(to_right,#414345,#232526)] p-3 rounded-lg hover:bg-pink-500 transition"> Submit </button>
